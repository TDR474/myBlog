...
- **Terminology**:
  - <ins>**Directed Edges vs Undirected**</ins>
    - Directed edges mean you can traverse in one direction, while undirected edges let you move in both directions.
      - For example, binary trees are directed graphs because you can't move back to a node's parent, only its children.
  - <ins>**Connected Component**</ins>:
    - This is a group of nodes connected by edges.
  - <ins>**Indegree**</ins>:
    - This is the number of edges that can be used to **reach** the node.
      - In graphs, nodes with an **in-degree of zero** are the **smallest set of nodes from which all other nodes can be reached**.
         - For instance, check out [1557. Minimum Number of Vertices to Reach All Nodes](https://leetcode.com/problems/minimum-number-of-vertices-to-reach-all-nodes/)
      - In binary trees, all nodes except the root have an indegree of 1 (due to their parent).
      - In DAGs (Directed Acyclic Graphs), a node with an **indegree of zero is called a <ins>source</ins>**.
  - <ins>**Outdegree**</ins>:
    - This is the number of edges that can be used to **leave** the node.
      - In binary trees, an outdegree of 0 means it's a leaf.
      - In binary trees, all nodes have an outdegree of 0, 1, or 2.
      - In DAGs, an **outdegree of zero is called a <ins>sink</ins>**.
        - Visiting from sink components will yield all strongly connected graphs (using Kosaraju's algorithm).
  - <ins>**Neighbors**</ins>: Nodes that are connected by an edge.
  - <ins>**Cyclic**</ins>:
    - When you have a path in the edges that **leads to visiting the same nodes infinitely**.
      - By definition, binary trees are acyclic.
